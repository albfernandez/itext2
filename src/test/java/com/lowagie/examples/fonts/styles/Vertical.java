...
public class Vertical {
	...
	static String texts[] = {
			"Some very long text to check if it wraps (or not).", " In blue.",
	...
	public void main() throws Exception {
		...
		PdfWriter writer = PdfWriter.getInstance(document,
				PdfTestBase.getOutputStream("vertical.pdf"));
		int idx = 0;
		document.open();
		...
		for (int j = 0; j < 2; ++j) {
			...
			vt.addText(new Chunk(texts[idx++],
					new Font(bf, 20, 0, Color.orange)));
			status = vt.go();
			System.out.println(status);
			document.newPage();
		}
