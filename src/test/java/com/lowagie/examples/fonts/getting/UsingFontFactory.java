...
public class UsingFontFactory {
	...
	public void main() throws Exception {
		...
		PdfWriter.getInstance(document,
				PdfTestBase.getOutputStream("FontFactory.pdf"));

		// step 3: we open the document
		document.open();
		// step 4:
		String name;
		Paragraph p = new Paragraph("Font Families", FontFactory.getFont(
		...
		p = new Paragraph("Fonts", FontFactory.getFont(FontFactory.HELVETICA,
				16f));
		for (Iterator i = families.iterator(); i.hasNext() && c > 0;) {
			...
			document.add(p);
			try {
				p = new Paragraph(quick, FontFactory.getFont(name,
