...
public class PatternsTest {
	...
	public void main() throws Exception {
		...
		PdfWriter writer = PdfWriter.getInstance(document,
				PdfTestBase.getOutputStream("patterns.pdf"));
		...
		// step 5: we create some PdfPatternPainter instances for drawing path,
		// text, or placing image
		...
		PdfPatternPainter p2 = cb.createPattern(img.getScaledWidth(),
		...
		p2.addImage(img, img.getScaledWidth(), 0f, 0f, img.getScaledHeight(),
				0f, 0f);
		...
		document.add(new Paragraph(text, FontFactory.getFont(
				FontFactory.HELVETICA, 60, Font.BOLD, new GrayColor(0.3f))));
		document.add(new Paragraph(text, FontFactory.getFont(
				FontFactory.HELVETICA, 60, Font.BOLD, pat)));
		...
		document.add(new Paragraph(text, FontFactory.getFont(
				FontFactory.HELVETICA, 60, Font.BOLD, new GrayColor(0.3f))));
		document.add(new Paragraph(text, FontFactory.getFont(
				FontFactory.HELVETICA, 60, Font.BOLD, pat1)));
		// draw a rectangle
		...
		document.add(new Paragraph(text, FontFactory.getFont(
				FontFactory.HELVETICA, 60, Font.BOLD, new GrayColor(0.3f))));
		document.add(new Paragraph(text, FontFactory.getFont(
				FontFactory.HELVETICA, 60, Font.BOLD, pat2)));
		// The original Image for comparison reason.
		// Note: The width and height is the same as bbox in pattern
		cb.addImage(img, img.getScaledWidth(), 0f, 0f, img.getScaledHeight(),
				350f, 400f);
		...
		// draw some characters.
		// Note: if the image fills up the pattern, there's no need to draw text
		// twice
